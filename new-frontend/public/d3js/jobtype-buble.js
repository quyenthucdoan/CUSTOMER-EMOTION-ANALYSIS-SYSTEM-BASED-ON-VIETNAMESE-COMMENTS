const JobTypeBuble = new function () {
	const CONFIG = {
		maxBubbleRadius: 20,
		kincaidDomainPadding: 0.1,
		xAxisLabel: '파견인원수',
		yAxisLabel: '평균임금',
		keyCircleLabel: 'Country',
	...
	this.init = (jobtype) => {
		...
		popupTemplate = _.template("<h3><%= speech.country %></h3><div class='country-buble-popup-date'>Average wage: <%= speech.avgStr %></div><div class='country-buble-popup-date'>Total submission: <%= speech.total_application %></div>");
		...
		d3.json('http://106.254.237.82:5052/jobtype/getCountryCorrelation?jobtype=' + jobtype, (json) => {
	...
	function renderPoints() {
		...
		texts
			...
			.attr('fill-opacity', (d) => {
				if (selectedLocation) {
					return d.country === selectedLocation.country ? 1 : 0.2;
				}
				return 0.7;
			});
	...
	function renderPopup() {
		if (selectedLocation) {
			...
			const leftPadding = Math.round(circleScale(selectedLocation.total_application)) + 26;
